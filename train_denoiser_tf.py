...
def load_model_for_continued_training(
    ...
    univa_model_path: Optional[str], #<-- 新增的 UNIVA/FLUX 权重路径
    device: str,
    dtype: torch.dtype,
):
    """
    ...
    执行流程:
    1. 加载原始的基础模型。
    2. (可选) 加载并更新来自 univa_model_path 的特定微调权重。
    3. 加载 LoRA 适配器，并保持其可训练状态。

    Args:
        model_class: 模型的类 (e.g., TFUnivaQwen2p5VLForConditionalGeneration)。
        base_model_path (str): 原始基础模型的路径。
        lora_adapter_path (str): 第一阶段训练好的 LoRA 权重的路径。
        univa_model_path (Optional[str]): 包含特定微调权重（如 denoise_tower）的 checkpoint 路径。
        device (str): 指定设备, 如 'cuda:0'。
        dtype (torch.dtype): 指定数据类型, 如 torch.bfloat16。

    Returns:
        PeftModel: 准备好进行下一阶段训练的模型。
    """
    # --- 步骤 1: 加载原始的基础模型 ---
    ...
    # --- 步骤 3: 附加 LoRA 适配器以继续训练 ---
    print(f"--- Step 3/3: Attaching LoRA Adapter from: {lora_adapter_path} ---")
    # is_trainable=True 确保适配器是可训练的
    # PEFT 会自动将适配器应用到基础模型的相应模块上
    if lora_adapter_path:
        peft_model = PeftModel.from_pretrained(base_model, lora_adapter_path, is_trainable=True)
        print("✅ LoRA adapter attached. Model is ready for continued training.")
        # 关键：我们不调用 .merge_and_unload() 或 .eval()
        # 我们需要保持 LoRA 层是分离的，以便 Trainer 可以找到并更新它们。
        # PeftModel 默认处于训练模式。
        print("\n🚀 Hybrid model loading for continued training is complete!")
    else:
        print("--- Step 3/3: Skipping LoRA adapter loading (no path provided). ---")
        peft_model = base_model # 如果没有LoRA，返回基础模型，或者根据实际需求处理

    return peft_model

# def load_model_for_continued_training(model_class, base_model_path: str, lora_adapter_path: str, device: str, dtype: str):
#     """
#     加载基础模型和现有的LoRA适配器，为下一阶段的训练做准备。
#     此函数返回一个PeftModel，其中的LoRA层是独立的，可以被训练器识别和更新。

#     Args:
#         base_model_path (str): 基础模型的路径。
#         lora_adapter_path (str): 第一阶段训练好的LoRA权重的路径。
#         device (str): 指定设备，如 'cuda:0'。

#     Returns:
#         PeftModel: 准备好进行下一阶段训练的模型。
#     """
#     print(f"--- Step 1/2: Loading Base Model from: {base_model_path} ---")
#     # 1. 加载基础模型
#     base_model = model_class.from_pretrained(
#         base_model_path,
#         torch_dtype=dtype,
...
#     print(f"--- Step 2/2: Attaching LoRA Adapter from: {lora_adapter_path} ---")
#     # 2. 加载并附加LoRA适配器
#     # is_trainable=True 确保适配器权重可以被训练（如果需要的话）
#     # PeftModel 会自动将适配器应用到基础模型的相应模块上
#     peft_model = PeftModel.from_pretrained(base_model, lora_adapter_path, is_trainable=True)
#     print("✅ LoRA adapter attached. The model is ready for further training.")
    
#     # 关键：我们不在这里调用 .merge_and_unload()
#     # 因为我们需要保持LoRA层是分离的，以便Trainer可以找到并更新它们。
#     return peft_model
...
def main(args: UnivaTrainingDenoiseConfig, attn_implementation='sdpa'):
    ...
    if dataset_type == 'qwen2vl' or dataset_type == 'qwen2p5vl' or dataset_type == 'qwen2p5vl_trans' or dataset_type == 'qwen2p5vl_tf' or 'lmdb_qwen':
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        for i, model in enumerate(models):
            ...
            # Case 1: 模型是 peft，保存 LoRA + 保存 base
            if isinstance(unwrapped, PeftModel):
                print("Saving LoRA adapter...")
                unwrapped.save_pretrained(os.path.join(output_dir, "lora"))

                print("Saving base model...")
                unwrapped.base_model.save_pretrained(os.path.join(output_dir, "univa"))
                processor.save_pretrained(os.path.join(output_dir, "univa"))

            # Case 2: 是原生模型
            elif isinstance(unwrapped, model_class):
                print("Saving full finetuned model...")
                unwrapped.save_pretrained(os.path.join(output_dir, "univa"))
                processor.save_pretrained(os.path.join(output_dir, "univa"))

            else:
                raise ValueError(f"Unexpected model type {type(unwrapped)}")
    ...
    dataset = dataset_class(
        ...
        use_step1x_preprocessing=False,  # Disable Step1X temporarily due to tokenizer issues
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(lvlm_model):
                ...
                if lm_loss is not None:
                    ...
                    total_loss = gen_loss_scalar + lm_loss_scalar
                else:
                ...
                print(f"[Step {global_step}] Loss -> Total: {log_dict['loss_total']:.4f}, "
                    f"Gen: {log_dict['loss_gen']:.4f}, LM: {log_dict['loss_lm']:.4f}")
