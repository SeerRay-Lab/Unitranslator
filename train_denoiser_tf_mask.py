...
from univa.utils.flux_pipeline import FluxKontextPipeline  # flux 
...
# from univa.utils.perception_loss import PerceptualLoss
...
lora_config = LoraConfig(
    ...
    task_type="CAUSAL_LM",
    # modules_to_save=[
        # "mask_tower",       # <-- 告诉PEFT，不要冻结 mask_tower
        # "mask_mlp",         # <-- 告诉PEFT，不要冻结 mask_mlp
    # ],
)
...
def _load_ckpt_state_dict(model_path: str):
    # 兼容 pytorch_model.bin / model.safetensors
    import os, glob
    st_files = glob.glob(os.path.join(model_path, "*.safetensors"))
    if st_files:
        # 常见是 model.safetensors / model-00001-of-000xx.safetensors（分片更复杂，下面给简单版）
        # 若是分片，需要用 transformers 的 load_sharded_checkpoint；这里先覆盖单文件情形
        return safe_load(st_files[0])
    bin_files = glob.glob(os.path.join(model_path, "pytorch_model*.bin"))
    if bin_files:
        return torch.load(bin_files[0], map_location="cpu")
    raise FileNotFoundError(f"No checkpoint weights found under: {model_path}")

def check_base_model_keys(base_model, base_model_path: str):
    sd = _load_ckpt_state_dict(base_model_path)
    missing, unexpected = base_model.load_state_dict(sd, strict=False)
    print(f"[BaseModel] missing_keys: {len(missing)}")
    print(f"[BaseModel] unexpected_keys: {len(unexpected)}")
    if missing[:20]:
        print("  missing sample:", missing[:20])
    if unexpected[:20]:
        print("  unexpected sample:", unexpected[:20])
def _load_lora_state_dict(lora_path: str):
    import os
    st = os.path.join(lora_path, "adapter_model.safetensors")
    bn = os.path.join(lora_path, "adapter_model.bin")
    if os.path.exists(st):
        from safetensors.torch import load_file
        return load_file(st)
    if os.path.exists(bn):
        return torch.load(bn, map_location="cpu")
    raise FileNotFoundError(f"No LoRA adapter weights found under: {lora_path}")

def check_lora_keys(peft_model, lora_adapter_path: str):
    sd = _load_lora_state_dict(lora_adapter_path)
    missing, unexpected = peft_model.load_state_dict(sd, strict=False)
    print(f"[LoRA] missing_keys: {len(missing)}")
    print(f"[LoRA] unexpected_keys: {len(unexpected)}")
    if missing[:30]:
        print("  missing sample:", missing[:30])
    if unexpected[:30]:
        print("  unexpected sample:", unexpected[:30])
def check_param_sanity(model):
    n_nan = 0
    n_inf = 0
    for name, p in model.named_parameters():
        if p.data.is_floating_point():
            if torch.isnan(p.data).any(): n_nan += 1
            if torch.isinf(p.data).any(): n_inf += 1
    print(f"[Sanity] tensors_with_nan={n_nan}, tensors_with_inf={n_inf}")
def report_trainable_params(model):
    ...
    for _, p in model.named_parameters():
        ...
        if p.requires_grad:
            trainable += num
    print(f"[Trainable] {trainable/total:.6%} ({trainable} / {total}) trainable")
def list_lora_modules(peft_model, limit=50):
    from peft.tuners.lora import LoraLayer
    hit = []
    for name, m in peft_model.named_modules():
        if isinstance(m, LoraLayer):
            hit.append(name)
    print(f"[LoRA Modules] count={len(hit)}")
    print("  sample:", hit[:limit])


def load_univa_weights_from_checkpoint(model, ckpt_path: str):
    """
    辅助函数：从给定的 checkpoint 目录中加载特定的模型权重（例如 'denoise_tower' 或 'visual' 部分），
    并更新到当前模型中。
    
    Args:
        model: 需要被更新权重的 PyTorch 模型。
        ckpt_path (str): 包含 .safetensors 或 .bin 权重文件的目录路径。
    ...
    all_files = glob.glob(os.path.join(ckpt_path, "*.safetensors")) + \
                glob.glob(os.path.join(ckpt_path, "pytorch_model*.bin")) # 兼容Hugging Face格式

    state_dict_to_load = {}
    print(f"🔍 Searching for weights in {len(all_files)} file(s) at: {ckpt_path}")

    for f_path in all_files:
        # 根据文件类型加载权重
        sd = load_file(f_path, device="cpu") if f_path.endswith(".safetensors") else torch.load(f_path, map_location="cpu")
        
        for key, value in sd.items():
            # 你可以修改这里的过滤条件，以加载你需要的任何权重
            # 沿用你示例中的 'denoise_tower.' 逻辑
            if "denoise_tower." in key or "mask_tower." in key:
                # 移除权重文件中可能存在的前缀，使其与当前模型结构匹配
                # 常见的Hugging Face PEFT checkpoint前缀是 "base_model.model."
                clean_key = key.replace("base_model.model.", "")
                state_dict_to_load[clean_key] = value

    if state_dict_to_load:
        print(f"--- Attempting to load {len(state_dict_to_load)} keys for UNIVA/FLUX component ---")
        
        # 使用 strict=False 来只加载匹配的权重，忽略不匹配或缺失的
        missing_keys, unexpected_keys = model.load_state_dict(state_dict_to_load, strict=False)

        # 打印详细的加载报告
        if missing_keys:
            print("\n❌ WARNING: Some weights of the model were not initialized from the UNIVA checkpoint:")
            # 只显示与我们尝试加载的部分相关的缺失权重
            relevant_missing = [k for k in missing_keys if any(part in k for part in ["denoise_tower"])]
            for key in relevant_missing[:5]: # 只打印前5个样本
                print(f"  - {key}")
            if len(relevant_missing) > 5:
                print(f"  ... and {len(relevant_missing) - 5} more.")

        if unexpected_keys:
            print("\n❌ WARNING: Some weights of the UNIVA checkpoint were not used:")
            for key in unexpected_keys[:5]: # 只打印前5个样本
                print(f"  - {key}")
            if len(unexpected_keys) > 5:
                print(f"  ... and {len(unexpected_keys) - 5} more.")

        if not relevant_missing and not unexpected_keys:
            print("✅ All targeted UNIVA checkpoint keys were successfully matched and loaded.")
    else:
        print("🤷 No UNIVA-specific weights found or loaded from the provided path.")
...
def load_model_for_continued_training(
    model_class,
    base_model_path: str,
    lora_adapter_path: Optional[str],   # now Optional
    univa_model_path: Optional[str],    # now Optional
    device: str,
    dtype: torch.dtype,
    # 下面这些可选参数你也可以不暴露出去，直接写死在函数里
    ...
    1. 加载基础模型。
    2. (可选) 加载 univa 权重。
    3. (可选) 加载已有 LoRA adapter；若未提供，则创建一个新的 LoRA config，
       使返回值仍为 PeftModel（即“原模型的 peft 格式”）。
    返回: PeftModel
    """
    ...
    # --- Step 2: (Optional) Load UNIVA/FLUX weights ---
    if univa_model_path:
        print(f"--- Step 2/3: Loading UNIVA/FLUX weights from: {univa_model_path} ---")
        base_model = load_univa_weights_from_checkpoint(base_model, univa_model_path)
        print("✅ UNIVA/FLUX weights loaded.")
    else:
        print("--- Step 2/3: Skipping UNIVA/FLUX weights loading (no path provided). ---")

    # --- Step 3: Attach existing LoRA OR create a new PEFT wrapper ---
    if lora_adapter_path:
        print(f"--- Step 3/3: Attaching EXISTING LoRA Adapter from: {lora_adapter_path} ---")
        peft_model = PeftModel.from_pretrained(
            base_model,
            lora_adapter_path,
            is_trainable=True,
        )
        print("✅ Existing LoRA adapter attached. Model is ready for continued training.")
        print("\n🚀 Hybrid model loading for continued training is complete!")
        return peft_model

    # 没给 lora_adapter_path：创建一个新的 LoRA config，让返回仍然是 PeftModel
    print("--- Step 3/3: No LoRA path provided. Creating a NEW trainable LoRA adapter wrapper. ---")

    # 你需要根据模型结构指定 target_modules
    # 常见例子（LLaMA系）：["q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj"]
    # Diffusion U-Net/DiT 系：通常是 attention 的 to_q/to_k/to_v/to_out 或 proj_in/proj_out 等


    peft_model = get_peft_model(base_model, lora_config)
    # 确保可训练（get_peft_model 默认 LoRA 参数是可训练的）
    peft_model.print_trainable_parameters()

    print("✅ New LoRA adapter created & attached. Returned model is PeftModel.")
    print("\n🚀 Hybrid model loading for continued training is complete!")
    return peft_model
...
def main(args: UnivaTrainingDenoiseConfig, attn_implementation='sdpa'):
    ...
    #     torch_dtype=dtype, 
    # )
    lvlm_model = load_model_for_continued_training(
        model_class, 
        args.model_config.pretrained_lvlm_name_or_path, 
        args.model_config.lora_lvlm_name_or_path, 
        args.model_config.univa_lvlm_name_or_path, 
        # args.model_config.pretrained_lvlm_name_or_path.rsplit("/", 1)[0] + "/lora",
        accelerator.device,
        dtype=dtype
    )
    accelerator.print(f'{lvlm_model}')

    print("--- Manually unfreezing the 'mask_tower' module for scratch training... ---")
    
    # PEFT 推荐使用 `set_adapter` 上下文管理器来确保操作的安全性
    with lvlm_model.disable_adapter():
        # 在禁用适配器模式下，我们可以安全地修改基础模型的参数
        num_params_unfrozen = 0
        for name, param in lvlm_model.base_model.named_parameters():
            if 'mask_tower' in name or 'mask_mlp' in name:
                param.requires_grad = True
                num_params_unfrozen += 1

    if num_params_unfrozen > 0:
        print(f"✅ Successfully unfroze {num_params_unfrozen} parameters in 'mask_tower'/'mask_mlp'.")
    else:
        print("[WARN] 'mask_tower'/'mask_mlp' not found or no parameters were unfrozen.")
    
    print("\\n--- Final trainable parameters: ---")
    lvlm_model.print_trainable_parameters()
    
    
    
    lvlm_tokenizer, image_processor, processor = None, None, None
    if dataset_type == 'qwen2vl' or dataset_type == 'qwen2p5vl' or dataset_type == 'qwen2p5vl_trans' or dataset_type == 'qwen2p5vl_tf' or dataset_type == 'qwen2p5vl_tf_mask':
    ...
    #         # --- 场景 1: 模型是 PeftModel (LoRA 训练) ---
    #         if isinstance(unwrapped, PeftModel):
    #             print("模型是 PeftModel，正在分别保存基础模型和 LoRA 适配器...")
                
    #             # 定义输出目录
    #             univa_output_dir = os.path.join(output_dir, "univa")
    #             lora_output_dir = os.path.join(output_dir, "lora")

    #             # 1. 【关键修复】使用 get_base_model() 获取最原始的基础模型
    #             # 无论有多少层 PEFT 包装，这都能直达核心
    #             base_model_to_save = unwrapped.get_base_model()
                
    #             print(f"正在保存真正的基础模型 (类型: {type(base_model_to_save)}) 到: {univa_output_dir}")
    #             base_model_to_save.save_pretrained(univa_output_dir)
                
    #             # 同时将 processor 的配置与基础模型保存在一起
    #             processor.save_pretrained(univa_output_dir)
    #             print("✅ 基础模型和 processor 已保存到 'univa' 目录。")

    #             # 2. 将最外层的 LoRA 适配器保存到 'lora' 目录
    #             # unwrapped 是最外层的 PeftModel，调用 save_pretrained 会保存它的适配器
    #             print(f"正在保存 LoRA 适配器到: {lora_output_dir}")
    #             unwrapped.save_pretrained(lora_output_dir)
    #             print("✅ LoRA 适配器已保存到 'lora' 目录。")

    #         # --- 场景 2: 模型是标准 PreTrainedModel (全量微调) ---
    #         elif isinstance(unwrapped, model_class):
    #             print("模型是标准 PreTrainedModel (全量微调)，正在保存所有权重...")
    #             univa_output_dir = os.path.join(output_dir, "univa")
    #             unwrapped.save_pretrained(univa_output_dir)
    #             processor.save_pretrained(univa_output_dir)
    #             print(f"✅ 完整模型和 processor 已保存到: {univa_output_dir}")

    #         # --- 异常情况 ---
    #         else:
    #             raise ValueError(f"提供了错误的模型类型。期望是 PeftModel 或 {model_class}, 但得到的是 {type(unwrapped)}。")
    ...
    def save_model_hook(models, weights, output_dir):
        """
        一个适用于多机多卡（FSDP/DeepSpeed）环境的、健壮的模型保存钩子。
        它能同时处理基础模型（包含新增模块）和LoRA适配器的保存。
        """
        
        # 关键修复 1: 移除主进程判断。所有进程都必须参与保存过程！
        # if not accelerator.is_main_process:
        #     return  <- 导致死锁的元凶，必须删除！

        # 为了防止文件系统操作的竞争，让主进程创建目录
        if accelerator.is_main_process:
            # 定义输出目录
            univa_output_dir = os.path.join(output_dir, "univa")
            lora_output_dir = os.path.join(output_dir, "lora")
            os.makedirs(univa_output_dir, exist_ok=True)
        ...
        for i, model in enumerate(models):
            ...
            # 解包模型，获取最外层的 PeftModel 或 PreTrainedModel
            unwrapped_model = accelerator.unwrap_model(model)

            # --- 场景 1: 模型是 PeftModel (LoRA 训练) ---
            if isinstance(unwrapped_model, PeftModel):
                print(f"进程 {accelerator.process_index}: 检测到 PeftModel，开始分布式保存...")

                # 步骤 1: 保存 LoRA 适配器（轻量级，只需主进程执行即可）
                if accelerator.is_main_process:
                    print("正在保存 LoRA 适配器...")
                    unwrapped_model.save_pretrained(lora_output_dir)
                    print("✅ LoRA 适配器已由主进程保存到 'lora' 目录。")
                
                # 步骤 2: 【关键修复】保存基础模型（包含您的新增模块）
                # 这部分是分片的，必须使用 accelerator 的 API 来处理！
                print(f"进程 {accelerator.process_index}: 准备保存分片的基础模型...")
                
                # 获取底层的、包含新增模块的基础模型
                base_model_to_save = unwrapped_model.get_base_model()
                
                # 使用 accelerator.save_model，它知道如何从所有进程收集分片并保存
                # safe_serialization=True 是推荐的最佳实践
                accelerator.save_model(base_model_to_save, univa_output_dir, safe_serialization=True)
                print(f"进程 {accelerator.process_index}: 参与基础模型保存完成。")

                # 步骤 3: 保存 processor (只需主进程执行)
                if accelerator.is_main_process:
                    processor.save_pretrained(univa_output_dir)
                    print("✅ 基础模型和 processor 已保存到 'univa' 目录。")

            # --- 场景 2: 模型是标准 PreTrainedModel (全量微调) ---
            elif isinstance(unwrapped_model, model_class):
                print(f"进程 {accelerator.process_index}: 检测到标准 PreTrainedModel，开始分布式保存...")
                
                # 同样，使用 accelerator.save_model 来安全地保存
                accelerator.save_model(unwrapped_model, univa_output_dir, safe_serialization=True)
            ...
            # --- 异常情况 ---
                ...
                # 错误应该在所有进程上都抛出
                raise ValueError(f"进程 {accelerator.process_index}: 提供了错误的模型类型。期望是 PeftModel 或 {model_class}, 但得到的是 {type(unwrapped_model)}。")

        # 再次同步，确保所有进程都完成了自己的任务，主进程已经将所有文件写入磁盘
        accelerator.wait_for_everyone()
        
        if accelerator.is_main_process:
            print("所有保存任务已在所有进程上完成。")
            if weights:
                weights.pop()
    ...
    # lvlm_model = get_peft_model(lvlm_model, lora_config)  # LoRA

    # print("--- Manually unfreezing the 'mask_tower' module... ---")
    
    # 找到最底层的、真正的模型
    # 根据您的报错路径，我们需要深入挖掘
    try:
        # 尝试常见的 PEFT 结构
        base_model_we_need = lvlm_model.base_model.model
    except AttributeError:
        # 如果结构不同，您可能需要根据实际情况调整
        print("[ERROR] Could not find the base model at peft_model.base_model.model. Please check the model structure.")
        # 在这里可以设置一个断点，`import pdb; pdb.set_trace()`，然后手动检查 `peft_model` 的结构
        base_model_we_need = lvlm_model 

    num_params_unfrozen = 0
    if hasattr(base_model_we_need, 'mask_tower'):
        for param in base_model_we_need.mask_tower.parameters():
            param.requires_grad = True
            num_params_unfrozen += 1
        print(f"Successfully unfroze {num_params_unfrozen} parameters in 'mask_tower'.")
    else:
        print("[WARN] 'mask_tower' not found in the base model. It will not be trained.")
    ...
    if args.model_config.with_tune_mlp2:
        for name, param in lvlm_model.named_parameters():
            if 'denoise_tower.fusion_layer' in name:
                param.requires_grad_(True)
            if 'denoise_tower.denoise_projector' in name:
                param.requires_grad_(True)
    ...
    if use_deepspeed_optimizer:
        ...
        if args.training_config.optimizer.lower() == "adamw":
            lr_finetune = args.training_config.learning_rate     # 用于 LoRA 精调的、极低的学习率
            lr_scratch = 4e-3       # 用于从零开始训练的新模块的、较高的学习率
            finetune_keywords = ["lora_", "denoise_tower"]
            scratch_keywords = ["mask_tower", "mask_mlp"]
            params_finetune = []
            params_scratch = []
            params_other = [] # 用于存放意外的可训练参数，进行调试

            # 4. 遍历所有可训练参数
            #    trainable_params 是你之前创建的、包含了所有 requires_grad=True 参数的列表
            for param in trainable_params:
                # 为了获取参数名，我们需要再次遍历 named_parameters
                # 这是一个稍微低效但100%可靠的方法
                found = False
                for name, p_check in lvlm_model.named_parameters():
                    if p_check is param: # 找到我们正在处理的这个param对象
                        ...
                        if any(keyword in name for keyword in scratch_keywords):
                            ...
                            break
                        # 检查是否属于 "精调" 组
                        elif any(keyword in name for keyword in finetune_keywords):
                            params_finetune.append(param)
                            found = True
                            break
                
                # 如果一个参数既不属于scratch也不属于finetune，就把它放到other组
                if not found:
            ...
            optimizer_grouped_parameters = [
                ...
                {
                    "params": params_scratch,
                    "lr": lr_scratch,
                    "weight_decay": args.training_config.adam_weight_decay,
                },
            ]

            # 如果 other 组里有东西，说明我们的分组逻辑有遗漏，必须打印出来
            if params_other:
                print("\n[WARN] Some trainable parameters were not assigned to any learning rate group!")
                optimizer_grouped_parameters.append({
                    "params": params_other,
                    "lr": lr_finetune, # 默认给一个低的LR
                })
                # 打印出这些被遗漏的参数的名字，以便调试
                for param in params_other:
                    for name, p_check in lvlm_model.named_parameters():
                        if p_check is param:
                            print(f"  -> Unassigned trainable param: {name}")
    ...
    # text_predictor = create_predictor().eval().to(accelerator.device)
    # recog_args = edict()
    # recog_args.rec_image_shape = "3, 48, 320"
    # recog_args.rec_batch_num = 6
    # recog_args.rec_char_dict_path = './ocr_recog/ppocr_keys_v1.txt'
    # recog_args.use_fp16 = True
    # cn_recognizer = TextRecognizer(recog_args, text_predictor)
    ...
    accelerator.print(f"  Num examples = {len(train_dataloader.base_dataloader.dataset)}")
    ...
    if hasattr(lvlm_model, 'mask_tower') and hasattr(lvlm_model.mask_tower, 'init_mgm_weights'):
        lvlm_model.mask_tower.init_mgm_weights()
    else:
        print("[WARN] Could not find `lvlm_model.mask_tower.init_mgm_weights`. MGM weights might not be correctly initialized.")
    ...
    for epoch in range(first_epoch, args.training_config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(lvlm_model):
                ...
                if args.training_config.drop_t5_rate <= random.random():
                    with torch.no_grad():
                        t5_prompt_embeds, _ = encode_prompt(
                            ...
                            tokenizers,
                            prompt=t5_prompt,  # the value of last turn, which is instruction
                            max_sequence_length=256,
                            device=accelerator.device,
                            num_images_per_prompt=1,
                        )
                ...
                model_output = lvlm_model(
                    input_ids=input_ids,
                    labels=labels,  # (batch, length)  # 加入labels
                ...
                model_pred = model_output['model_pred']
                und_loss = model_output['loss']

                predicted_mask = model_output.get("predicted_mask")
                mask_loss = torch.tensor(0.0, device=accelerator.device)
                
                if predicted_mask is not None and "ground_truth_mask" in batch:
                    gt_masks_batch = batch["ground_truth_mask"].to(device=accelerator.device) # Shape: [B, NumMasks, 1, H, W]
                    if gt_masks_batch.numel() > 0:
                        target_mask = gt_masks_batch[:, 1, :, :, :].to((predicted_mask.dtype))  # target mask
                        loss_bce = F.binary_cross_entropy(predicted_mask, target_mask, reduction='mean')
                        epsilon = 1e-6
                        intersection = (predicted_mask * target_mask).sum(dim=(1,2,3)) # 在 C, H, W 维度上求和
                        denominator = predicted_mask.sum(dim=(1,2,3)) + target_mask.sum(dim=(1,2,3))
                        
                        # Dice Coefficient
                        dice_coeff = (2. * intersection + epsilon) / (denominator + epsilon)
                        
                        # Dice Loss
                        loss_dice = 1 - dice_coeff.mean() # 对batch中的所有样本取平均
                        
                        # c. 组合Loss
                        #    通常将两者相加。你可以为它们设置不同的权重。
                        mask_loss = 0.1 * (loss_bce + loss_dice)    
                ...
                # -------------------------------
                # 1️⃣ 生成损失（Diffusion / denoiser loss）
                # -------------------------------
                gen_loss = (weighting.float() * (model_pred.float() - target.float()) ** 2).reshape(target.shape[0], -1)

                # pred_original_latent = (noisy_model_input - sigmas * model_pred) / (1.0 - sigmas.clamp(max=1 - 1e-6))
                # pred_image = vae.decode(
                #     pred_original_latent / vae.config.scaling_factor + vae.config.shift_factor
                # ).sample

                # gt_original_latent = (noisy_model_input - sigmas * target) / (1.0 - sigmas.clamp(max=1 - 1e-6))
                # gt_image = vae.decode(
                #     gt_original_latent / vae.config.scaling_factor + vae.config.shift_factor
                # ).sample

                # ocr_target_h, ocr_target_w = 256, 256 # 从 recog_args.rec_image_shape = "3, 48, 320" 获取

                # # 假设 VAE 输出的图像在 [-1, 1] 范围，先转换到 [0, 1]
                # pred_image_norm_0_1 = (pred_image + 1) / 2
                # gt_image_norm_0_1 = (gt_image + 1) / 2

                # # 调整大小
                # pred_image_resized = F.interpolate(pred_image_norm_0_1, size=(ocr_target_h, ocr_target_w), mode='bicubic', align_corners=False)
                # gt_image_resized = F.interpolate(gt_image_norm_0_1, size=(ocr_target_h, ocr_target_w), mode='bicubic', align_corners=False)

                
                # pred_ocr_feat = text_predictor(pred_image_resized)['ctc']
                # gt_ocr_feat = text_predictor(gt_image_resized)['ctc']

                # ocr_loss = ((pred_ocr_feat.float() - gt_ocr_feat.float()) ** 2).reshape(target.shape[0], -1).mean()

                # -------------------------------
                # 2️⃣ 理解损失（Language / LM loss）
                # -------------------------------
                if und_loss is not None:
                    lm_loss = 0.1 * und_loss  # 按设定权重
                else:
                    lm_loss = None
                
                # -------------------------------
                # 3️⃣ 对 batch 进行归一化 / 加权
                # -------------------------------
                if weight_mask is not None:
                    assert args.dataset_config.batch_size != 1
                    gen_loss_scalar = gen_loss.sum() / weight_mask.sum() / model_pred.shape[1]
                else:
                    gen_loss_scalar = gen_loss.mean()

                if lm_loss is not None and mask_loss is not None:
                    # und_loss 本身已经是 scalar，如果是 per-token 可以再 .mean()
                    lm_loss_scalar = lm_loss.mean() if lm_loss.ndim > 0 else lm_loss
                    mask_loss_scalar = mask_loss.mean() if mask_loss.ndim > 0 else mask_loss
                    total_loss = gen_loss_scalar + lm_loss_scalar + mask_loss_scalar
                else:
                    lm_loss_scalar = torch.tensor(0.0, device=gen_loss_scalar.device)
                    total_loss = gen_loss_scalar

                # -------------------------------
                # 4️⃣ 多卡聚合
                # -------------------------------
                avg_gen_loss_list = accelerator.gather(gen_loss_scalar)
                avg_lm_loss_list = accelerator.gather(lm_loss_scalar)
                avg_total_loss_list = accelerator.gather(total_loss)
                avg_mask_loss_list = accelerator.gather(mask_loss_scalar)
                # -------------------------------
                # 5️⃣ 梯度反传
                # -------------------------------
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(trainable_params, args.training_config.max_grad_norm)

                # -------------------------------
                # 7️⃣ 打印 / 日志记录
                # -------------------------------
                log_dict = {
                    "loss_total": avg_total_loss_list.mean().item(),
                    "loss_gen": avg_gen_loss_list.mean().item(),
                    "loss_lm": avg_lm_loss_list.mean().item(),
                    "loss_mask": avg_mask_loss_list.mean().item(),
                    # "loss_per": avg_per_loss_list.mean().item(),
                }
                print(f"[Step {global_step}] Loss -> Total: {log_dict['loss_total']:.4f}, "
                    f"Gen: {log_dict['loss_gen']:.4f}, LM: {log_dict['loss_lm']:.4f}, Mask: {log_dict['loss_mask']:.6f}")
            ...
            if global_step % log_interval == 0:
                logs = {
                    ...
                    "loss": avg_total_loss_list.mean().detach().item(), 
