...
class MaskUnivaQwen2p5VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not only_use_t5:
            ...
            final_hidden_states = all_hidden_states[-1]   # 最后一层，用于LM和Denoise
            hidden_states_for_mask = [
                all_hidden_states[9],
                all_hidden_states[18],
                all_hidden_states[27],
                all_hidden_states[36]
            ]
            # all_hidden_states = outputs.hidden_states     # tuple of all hidden states
            # final_hidden_states = all_hidden_states[-1]   # 最后一层，用于LM和Denoise
        ...
        if output_type.startswith("denoise"):
            ...
            if output_type == "denoise_embeds":
                ...
                # 我们需要获取 image_begin_token_id 和 image_end_token_id
                # 假设它们存储在config中
                image_begin_id = self.config.image_begin_token_id # 你需要确认这个路径
                image_end_id = self.config.image_end_token_id     # 你需要确认这个路径
                image_pad_id = self.config.image_token_id   
                selected = (input_ids == self.config.image_token_id)

                for i in range(input_ids.shape[0]):
                    sample_input_ids = input_ids[i]
                    
                    # 找到所有 image_begin_token 的位置
                    start_indices = (sample_input_ids == image_begin_id).nonzero(as_tuple=True)[0]
                    # 找到所有 image_end_token 的位置
                    end_indices = (sample_input_ids == image_end_id).nonzero(as_tuple=True)[0]

                    if len(start_indices) != len(end_indices):
                        print(f"[WARN] Mismatch between image start ({len(start_indices)}) and end ({len(end_indices)}) tokens. Skipping mask prediction for this sample.")
                        continue

                    # 假设它们是成对出现的
                    for start_idx, end_idx in zip(start_indices, end_indices):
                        # 图像块的起始位置，是 <vision_start> 之后的位置
                        image_block_start = start_idx.item() + 1
                        # 图像块的结束位置，是 <vision_end> 的位置
                ...
                if self.mask_tower is not None and ground_truth_mask is not None and image_positions:
                    # ✨ 使用新的 Mask Tower ✨
                    start_text_idx = [i for i, x in enumerate(input_ids[0]) if x == self.config.image_token_id][-1] + 1
                    predicted_mask = self.mask_tower(
                        list_of_hidden_states=all_hidden_states,
                        image_positions=image_positions, # 来自Dataset
                ...
                # --- ✨ 核心修改：同时返回 logits ✨ ---
                return {"loss": loss, "model_pred": model_pred, "logits": logits,"token_acc": token_acc, "predicted_mask": predicted_mask}
