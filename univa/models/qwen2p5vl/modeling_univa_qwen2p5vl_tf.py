...
class TFUnivaQwen2p5VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not only_use_t5:
            ...
            if self.training and use_teacher_forcing and labels is not None:
                # <TF-MOD-CORE-LOGIC>:
                # 如果是训练模式、启用了Teacher Forcing、且有GT labels，
                # 我们就构建一个“完美”的条件来代替LVLM的输出。

                # a. 复制一份 LVLM 的输出，我们仍然需要它来计算语言损失
                hidden_states_for_denoiser = hidden_states_from_lvlm.clone()
                
                # b. 从 labels 创建 Ground Truth 文本嵌入
                # 我们需要忽略 labels 中的 -100 (padding/masked tokens)
                gt_labels = labels.clone()
                gt_labels[gt_labels == -100] = self.config.image_token_id # 用pad token id替换-100
                gt_embeds = self.model.embed_tokens(gt_labels)

                # c. 找到 labels 中非-100的部分，只在这些位置上进行替换
                #    这确保我们只替换文本部分，而不影响图像嵌入部分
                text_mask = (labels != -100).unsqueeze(-1).expand_as(hidden_states_for_denoiser)
                
                # d. 将GT嵌入注入到条件中
                #    masked_scatter_ 会在 text_mask 为 True 的地方，用 gt_embeds 的值替换
                hidden_states_for_denoiser.masked_scatter_(text_mask, gt_embeds)

            else:
                # <TF-MOD-CORE-LOGIC>:
                # 在推理模式或不使用Teacher Forcing时，直接使用LVLM的输出
                hidden_states_for_denoiser = hidden_states_from_lvlm
            # --- ✨ Teacher Forcing 结束 ✨ ---


            if output_type.startswith("denoise"):
                # <TF-MOD-CHANGE>: 将 denoise tower 的输入从 hidden_states 改为我们处理过的 hidden_states_for_denoiser
                
                # ... [vlm_residual_image_factor 和 shortcut_image_embeds 的逻辑保持不变，但操作对象是 hidden_states_for_denoiser] ...
                # (此处省略)
                
                # 将（可能被强制过的）条件投射到FLUX所需的维度
                projected_embeds = self.denoise_tower.denoise_projector(hidden_states_for_denoiser)

                if output_type == "denoise_embeds":
                    # ... (省略)
                    # 关键：语言损失仍然使用未经修改的 LVLM 输出计算
                    logits = self.lm_head(hidden_states_from_lvlm)
                    loss = compute_language_loss(logits, labels) if labels is not None else None
                    return {"loss": loss, "prompt_embeds": projected_embeds}

                elif output_type == "denoise_model_pred":
                    denoiser_kwargs["enc_attention_mask"] = attention_mask
                    # `forward_denoise_tower` 接收的是被强制过的 `projected_embeds`
                    model_pred = self.forward_denoise_tower(projected_embeds, **denoiser_kwargs)
                    
                    # 关键：计算语言损失(LM loss)时，必须使用【原始的、未经强制的】LVLM输出
                    logits = self.lm_head(hidden_states_from_lvlm)
                    loss = compute_language_loss(logits, labels) if labels is not None else None
                    token_acc = 0.0
