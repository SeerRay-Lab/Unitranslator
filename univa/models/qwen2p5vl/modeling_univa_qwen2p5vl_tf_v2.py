...
from univa.models.qwen2p5vl.configuration_univa_qwen2p5vl_v2 import UnivaQwen2p5VLConfig
        ...
        if not only_use_t5:
            ...
            # --- ✨ Teacher Forcing 核心干预点 ✨ ---
                ...
                if output_type == "denoise_embeds":
                    ...
                    # 4. ✨ 修改：替换 denoise_tower 的调用方式
                    #    不再使用 forward_denoise_tower 辅助函数，而是直接调用 self.denoise_tower
                    #    这样我们可以明确地传入两个独立的上下文
                    denoiser_kwargs["enc_attention_mask"] = attention_mask # 确保mask正确传递
                    model_pred = self.denoise_tower(
                        text_query=text_query,
                        context_kv=context_kv,
                        **denoiser_kwargs,
                    ...
                    # e. 计算语言损失 (关键：必须使用原始的、未经解码-重编码的 LVLM 输出)
                    ...
                    return {"loss": loss, "model_pred": model_pred, "logits": logits_for_loss, "token_acc": token_acc}
