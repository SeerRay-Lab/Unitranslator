...
from univa.models.qwen2p5vl.configuration_univa_qwen2p5vl_wmask import UnivaQwen2p5VLConfig
from univa.models.modeling_univa_denoise_tower_v2 import UnivaDenoiseTower
from torch.utils.checkpoint import checkpoint
from univa.models.modeling_univa_mask_tower import UnivaMaskTower
from univa.models.configuration_univa_mask_tower import UnivaMaskTowerConfig
...
class MaskTFUnivaQwen2p5VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: UnivaQwen2p5VLConfig):
        ...
        mask_tower_config_obj = config.mask_tower_config
        if mask_tower_config_obj is not None:
             self.mask_tower = UnivaMaskTower(mask_tower_config_obj)
             print("✅ UnivaMaskTower initialized successfully using pre-existing config object.")
        else:
             print("[WARN] `mask_tower_config` is None. Mask Tower will not be created.")
             self.mask_tower = None
    ...
    def forward(
        ...
        # <TF-MOD-ADD>: 新增一个参数来控制是否启用Teacher Forcing
        ...
        **kwargs,
        ...
        if not only_use_t5:
            ...
            # 最后一层，仍然用于计算语言损失 (loss)
            hidden_states_from_lvlm = all_hidden_states[-1] 
            # hidden_states_from_lvlm = outputs[0]
            hidden_states = hidden_states_from_lvlm
            
            if output_type.startswith("denoise"):
                outputs = outputs[0]  # (B, N, C)
            # --- ✨ Teacher Forcing 核心干预点 ✨ ---
                if use_teacher_forcing and labels is not None:
                    # --- 训练模式: 使用 GT Labels ---
                    # logger.info("Using Teacher Forcing for Denoiser Query.")
                    gt_labels = labels.clone()
                    # 将 -100 替换为 pad_token_id，使其可以通过 embed_tokens
                    # 注意: pad_token_id 不应影响注意力计算，因为它会被 attention_mask 覆盖
                    gt_labels[gt_labels == -100] = self.config.pad_token_id if self.config.pad_token_id is not None else 0
                    
                    # 直接从 GT labels 生成完美的文本嵌入
                    text_query_unprojected = self.model.embed_tokens(gt_labels)

                else:
                    # --- 推理模式: 解码 -> 重编码 ---
                    predicted_ids = input_ids.clone()
                    text_query_unprojected = self.model.embed_tokens(predicted_ids)

                if output_type == "denoise_embeds":
                    
                    # a. 准备 K 和 V (画布): 永远是 LVLM 的完整输出
                    context_kv_unprojected = hidden_states_from_lvlm

                    # c. 将 Q 和 K/V 投影到 denoiser 的特征空间
                    text_query = text_query_unprojected
                    context_kv = context_kv_unprojected
                    
                    # d. 准备 pooled_projections，用于 timestep 条件
                    #    一个常见的策略是使用 Q (纯文本信号) 的平均池化
                    # pooled_projections = text_query.mean(dim=1)
                    # denoiser_kwargs["enc_attention_mask"] = attention_mask
                    # model_pred = self.denoise_tower(
                    #     text_query=text_query,
                    #     context_kv=context_kv,
                    #     **denoiser_kwargs,
                    # )
                    # fused_features = self.denoise_tower.fusion_layer(query_states=text_query,
                    # kv_states=context_kv,
                    # )

                    query_mask = (labels != -100)
                    kv_mask = (attention_mask == 1)   # 你的 context_kv 对应的 mask
                    fused_features = self.denoise_tower.fusion_layer(
                        query_states=text_query,
                        kv_states=context_kv,
                        query_mask=query_mask,
                        kv_mask=kv_mask,
                    )
                    fused_features = self.denoise_tower.denoise_projector(fused_features)
            

                    # f. 返回一个包含所有“原料”的字典
                    return {
                        "model_pred": fused_features,
                        "hidden_states": fused_features,
                        
                    }
                elif output_type == "denoise_model_pred":
                    
                    # --- 路径 A: 计算 denoise/FLUX 的 model_pred (保持不变) ---
                    # 这个调用是成功的，是我们的“黄金标准”。
                    context_kv_for_denoise = hidden_states_from_lvlm
                    text_query_for_denoise = text_query_unprojected

                    denoiser_kwargs["enc_attention_mask"] = attention_mask
                    
                    model_pred = self.denoise_tower(
                        text_query=text_query_for_denoise,
                        context_kv=context_kv_for_denoise,
                        **denoiser_kwargs,
                    )
                    
                    # --- 核心任务：计算语言模型损失 (保持不变) ---
                    logits_for_loss = self.lm_head(hidden_states_from_lvlm)
                    loss = compute_language_loss(logits_for_loss, labels) if labels is not None else None
                    token_acc = compute_token_accuracy_CAUSAL(logits_for_loss, labels) if logits_for_loss is not None and labels is not None else 0.0
                    
                    # --- 路径 B: 专门为分割任务创建高质量特征并计算 predicted_mask ---
                    predicted_mask = None
                    if self.mask_tower is not None and ground_truth_mask is not None:
                        
                        # ✨ 1. 准备分割任务的“食材”
                        intermediate_layer_idx = self.config.num_hidden_layers - 1
                        # 食材1: 包含空间信息的 MLLM 中间层特征 (完整的)
                        mllm_intermediate_features = all_hidden_states[intermediate_layer_idx]
                        # 食材2: 最精确的文本引导
                        teacher_forced_text_features = text_query_unprojected

                        # 4. 准备调用 Mask Tower 所需的图像位置信息 (您的原始逻辑，保持不变)
                        image_positions = []
                        image_token_lengths = []
                        image_begin_id = self.config.image_begin_token_id
                        image_end_id = self.config.image_end_token_id
                        selected = (input_ids == self.config.image_token_id)
                        
                        for i in range(input_ids.shape[0]):
                            sample_input_ids = input_ids[i]
                            start_indices = (sample_input_ids == image_begin_id).nonzero(as_tuple=True)[0]
                            end_indices = (sample_input_ids == image_end_id).nonzero(as_tuple=True)[0]
                            
                            if len(start_indices) != len(end_indices):
                                print(f"[WARN] Mismatch between image start ({len(start_indices)}) and end ({len(end_indices)}) tokens.")
                                continue
                            
                            for start_idx, end_idx in zip(start_indices, end_indices):
                                image_block_start = start_idx.item() + 1
                                image_block_end = end_idx.item()
                                image_block_length = image_block_end - image_block_start
                                if image_block_length > 0:
                                    image_positions.append(image_block_start)
                                    image_token_lengths.append(image_block_length)
                        
                        # 5. 使用新的、高质量的融合特征进行分割
                        if image_positions:
                            start_text_idx = [i for i, x in enumerate(input_ids[0]) if x == self.config.image_token_id][-1] + 1
                            
                            predicted_mask = self.mask_tower(
                                # 不再使用 list_of_hidden_states
                                mllm_intermediate_features=mllm_intermediate_features, # 传入原始中间层
                                teacher_forced_text_features=teacher_forced_text_features, # 传入GT文本特征
                                image_positions=image_positions,
                                image_token_lengths=image_token_lengths,
                                target_height=ground_truth_mask.shape[-2],
                                target_width=ground_truth_mask.shape[-1],
                                selected=selected,
                                pixel_values=pixel_values,
                                start_text_idx=start_text_idx
                            )

                    return {"loss": loss, "model_pred": model_pred, "logits": logits_for_loss, "token_acc": token_acc, "predicted_mask": predicted_mask}
            ...
            #     outputs = self.denoise_tower.denoise_projector(outputs)  # 2-layer mlp
            #     # VAE latents are handled directly in training loop via concatenation
            #     # No vae_projector or siglip_projector needed
