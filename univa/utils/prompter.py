...
class Qwen2Prompter(Prompter):
    def __init__(self):
        ...
        self.role_list = ["user", "assistant", "system", "ocr"]
...
class Qwen2VLPrompter(Prompter):
    def __init__(self):
        ...
        self.role_list = ["user", "assistant", "system", "ocr", "translation"]

        self.assistant_role = "assistant"
        self.system_role = "system"
        self.ocr_role = "ocr"
        self.translation_role = "translation"
        self.user_role = "user"
        self.default_system_prompt = "You are a helpful assistant."
    ...
    def get_train_prompt(self, data: list[dict]) -> list[dict]:
        ...
        for idx, item in enumerate(data):
            ...
            elif item["from"] == self.translation_role:
                prompt_list.append(
                    {
                        "prompt": f"{self.bos_token}{item['value']}{self.eos_token}",
                        "is_labels": True,
                        "from": item["from"],
                    }
                )
            elif item["from"] == self.ocr_role:
                continue
            else:
                raise ValueError(f"Role {item['from']} is not in the role list")
...
PROMPT_TYPE = {
    ...
    'qwen2p5vl_trans': Qwen2VLPrompter, 
    'qwen2p5vl_mask': Qwen2VLPrompter, 
    'qwen2p5vl_trans_cond': Qwen2VLPrompter,
    'qwen2p5vl_tf': Qwen2VLPrompter,
    'qwen2p5vl_cl': Qwen2VLPrompter,
    'qwen2p5vl_tf_mask': Qwen2VLPrompter,
    'lmdb_qwen': Qwen2VLPrompter
}