...
class Qwen2VLDataset_trans(Dataset):
    def __init__(
        self,
        dataset_type: str,
        data_txt: str,
        ...
        assert dataset_type == 'qwen2vl' or  dataset_type == 'qwen2p5vl_tf' or dataset_type == 'qwen2p5vl' or dataset_type == 'qwen2p5vl_trans' or dataset_type == 'qwen2p5vl_trans_cond' or dataset_type == 'qwen2p5vl_cl'
    ...
    #         # if role == self.prompter.ocr_role:
    #         #     # 将原文本转成 <ocr>...</ocr>
    #         #     conversations[-1]["value"] = f"<ocr>{conversations[-1]['value']}</ocr>"

    #         if role == self.prompter.translation_role:
    #             # 将原文本转成 <translation>...</translation>
    #             conversations[-1]["value"] = f"<translation>{conversations[-1]['value']}</translation>"
    ...
    #         # if item["from"] == self.prompter.ocr_role:
    #         #     # Normalize formatting: remove OpenAI-style tags if any
    #         #     item["prompt"] = item["prompt"].replace("<|im_start|>ocr\n", "")
    #         #     item["prompt"] = item["prompt"].replace("<|im_end|>", "")
    #             # Enforce unified format
    #             # item["prompt"] = f"<ocr>{item['prompt'].strip()}</ocr>"

    #         # elif item["from"] == self.prompter.translation_role:
    #         #     item["prompt"] = item["prompt"].replace("<|im_start|>translation\n", "")
    #         #     item["prompt"] = item["prompt"].replace("<|im_end|>", "")
    #             # item["prompt"] = f"<translation>{item['prompt'].strip()}</translation>"
    ...
    #     labels: Optional[torch.Tensor] = None,
    # ):
    #     # Find the indices of the single placeholder image token
    #     image_token_indices = (input_ids == image_token_id).nonzero(as_tuple=True)
    ...
    #     # Iterate through each placeholder and expand it
    #     for idx in image_token_indices[1]:
    #         image_token_length = image_token_lengths[cur_i]
    #         adjusted_idx = idx + offset

    #         assert input_ids[0, adjusted_idx] == image_token_id, "Mismatch: Expected image_token_id at adjusted index."

    #         # --- 1. Expand input_ids (This part is correct) ---
    #         input_ids = torch.cat(
    #             [
    #                 input_ids[:, :adjusted_idx],
    #                 input_ids.new_full((1, 1), image_begin_token_id),  # <|vision_start|>
    #                 input_ids.new_full((1, image_token_length), image_token_id), # <image>...<image>
    #                 input_ids.new_full((1, 1), image_end_token_id),  # <|vision_end|>
    #                 input_ids[:, adjusted_idx + 1:],
    #             ],
    #             dim=1,
    #         )

    #         # --- 2. Modify labels with the CORRECT logic ---
    #         if labels is not None:
    #             # The total length of the inserted block is 1 (begin) + length + 1 (end)
    #             total_inserted_length = 1 + image_token_length + 1
                
    #             # Create a mask of -100 for the entire block
    #             ignore_block = labels.new_full((1, total_inserted_length), -100)
                
    #             labels = torch.cat(
    #                 [
    #                     labels[:, :adjusted_idx],
    #                     ignore_block,  # <-- ✨ 核心修复：插入一整个 -100 块
    #                     labels[:, adjusted_idx + 1:],
    #                 ],
    #                 dim=1,
    #             )

    #         # --- 3. Update positions and offsets (This part is correct) ---
    #         # The position for attention is the start of the actual image tokens
    #         image_position.append((adjusted_idx + 1).item())
            
    #         # Total added length is (1 + length + 1) - 1 (original token)
    #         offset += (1 + image_token_length + 1) - 1
    #         cur_i += 1
